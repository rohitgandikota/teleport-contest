...
import { MON_POLE_DIST, OBJ_FLOOR, RAY, MFAST, NON_PM, W_ARMG, W_WEP,
    ...
    TEMPLE, RLOC_MSG, NC_SHOW_MSG, NO_NC_FLAGS
...
import { newsym, canseemon, canspotmon, sensemon, pline,
...
// src/monmove.c:2377 vamp_shift() — this is called when a vampire turns into
// a fog cloud in order to move under a closed door; if it was sensed via
// telepathy or seen via infravision, its new fog cloud shape will disappear.
async function vamp_shift(mon, ptr, domsg) {
    ...
    if (mon.data === ptr) {
        ...
        reslt = await newcham(mon, ptr, domsg ? NC_SHOW_MSG : NO_NC_FLAGS);
...
export async function m_move(mtmp, after) {
    ...
    if (mtmp.mtame)
        return await postmov(mtmp, ptr, omx, omy,
                             await dog_move(mtmp, after), seenflgs, can_tunnel);
    ...
    return await postmov(mtmp, ptr, omx, omy, mmoved, seenflgs, can_tunnel);
}
...
async function postmov(mtmp, ptr, omx, omy, mmoved, seenflgs, can_tunnel) {
    ...
    if (mmoved === MMOVE_MOVED) {
        ...
        /* sequencing issue: the monster is already on the door square, but
           the transformation and its message (when in sight) have not
           happened yet; move it back to its previous location for the
           vamp_shift() so the message happens at the right time, then
        ...
        if (is_vampshifter(mtmp) && !amorphous(mtmp.data)
            ...
            if (await vamp_shift(mtmp, game.mons[PMNAMES.PM_FOG_CLOUD],
                                 (seenflgs & 1) !== 0))
