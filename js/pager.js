...
export async function doextversion() {
    /* do_runtime_info() -> get_lua_version() creates a Lua state the FIRST
       time (cached in gl.lua_ver afterwards); the nhlib.lua align shuffle
       is its observable cost */
    if (!game._lua_ver_known) {
        game._lua_ver_known = true;
        nhl_init();
    }

    /* the version banner, then dat/options (js/version_data.js) */
    const win = tty_create_nhwindow(NHW_TEXT);
...
// src/pager.c:2577 dowhatdoes_core() — "%-8s%s." with key2txt of the key,
// or null when the key is not bound to a command.
export function dowhatdoes_core(q) {
    const ec_desc = key2extcmddesc(q);

    if (ec_desc !== null) {
        /* note: if "%-8s" gets changed, the "%8.8s" in dowhatdoes() will
           need a comparable change */
        return `${key2txt(q).padEnd(8)}${ec_desc}.`;
    }
    return null;
}
...
export async function dowhatdoes() {
    ...
    const line = dowhatdoes_core(q.charCodeAt(0));
    if (line !== null) {
