...
export function decodeScreen(screen) {
        ...
        while (i < line.length) {
            ...
            if (ch === '\x1b') {
                ...
                if (line[i + 1] === '[') {
                    ...
                    if (fin === 'C') out += ' '.repeat(parseInt(params, 10) || 1);
                    // 'm' (SGR) and anything else: stripped
                    i = j + 1;
                    continue;
                }
                i += 1;
            ...
            if (ch === '\x0e') { dec = true; i += 1; continue; }
            if (ch === '\x0f') { dec = false; i += 1; continue; }
    ...
    return rows.slice(0, 24).map((r) => (r + ' '.repeat(80)).slice(0, 80));
}
